...
public class BSTMap<K, V> implements Map<K, V> {
	...
	public V put(K key, V value) {
		...
		if (result != null)  rtr=result.getElement().getValue(); 
	...
	public V remove(K key) {
		...
		tree.getPosition(new MapEntry<>(key, null))		;
		Entry<K, V> result=tree.removeElement(tree.getPosition(new MapEntry<>(key, null)).getElement());
		
		return result.getValue();    // for the moment...
	}
	...
	public Iterable<V> values() {
		...
		// inner class......  EXERCISE 2
		//...
		LinkedList<V> keySet=new LinkedList<>();
		for(Entry<K,V> x:tree) {
